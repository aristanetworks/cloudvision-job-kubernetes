...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo "  --image-name NAME               Docker image name (default: cv-job-informer)"
      echo "  --image-tag TAG                 Docker image tag (default: latest)"
      echo "  --registry REGISTRY             Container registry (e.g., myregistry.io)"
...
if [ "$FULL_IMAGE_NAME" != "cv-job-informer:latest" ]; then
...
if [ "$NODECONFIG_MODE" = "discovery" ]; then
    ...
    sed -e "s|image: cv-job-informer:latest|image: $FULL_IMAGE_NAME|g" \
